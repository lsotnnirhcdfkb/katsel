...
condition_to_sections (XIsMissingYFound x y sp (Located _ tok)) =
    (
        [ MsgUnds.Underline sp MsgUnds.Primary [MsgUnds.Message MsgUnds.Error $ x ++ " is missing " ++ y ++ " (found " ++ Tokens.format_token tok ++ " instead)"]
        ],
    [])
condition_to_sections (XIsMissingYAfterZFound x y z sp (Located _ tok)) =
    (
        [ MsgUnds.Underline sp MsgUnds.Primary [MsgUnds.Message MsgUnds.Error $ x ++ " is missing " ++ y ++ " after " ++ z ++ " (found " ++ Tokens.format_token tok ++ " instead)"]
        ],
    [])
condition_to_sections (ExcessTokens sp (Located _ tok)) =
    (
        [ MsgUnds.Underline sp MsgUnds.Primary [MsgUnds.Message MsgUnds.Error $ "extraneous tokens found in input (found " ++ Tokens.format_token tok ++ ")"]
        ],
    [])
condition_to_sections (InvalidToken construct thing possibilities sp (Located _ found)) =
    (
        [ MsgUnds.Underline sp MsgUnds.Primary [MsgUnds.Message MsgUnds.Error $ "invalid " ++ thing ++ " for " ++ construct ++ "; must be " ++ format_possibilities possibilities ++ " (found " ++ Tokens.format_token found ++ " instead)"]
        ],
    [])
    where
...
condition_to_sections (Unclosed construct delimiter_name open_span sp (Located _ found)) =
    (
        [ MsgUnds.Underline sp MsgUnds.Primary [MsgUnds.Message MsgUnds.Error $ construct ++ " has unclosed " ++ delimiter_name ++ " (found " ++ Tokens.format_token found ++ " instead)"]
        , MsgUnds.Underline open_span MsgUnds.Secondary [MsgUnds.Message MsgUnds.Note $ "opening " ++ delimiter_name ++ " is here"]
        ],
    [])
condition_to_sections (IfContinuedNeeds listname delimiter item sp (Located _ found)) =
    (
        [ MsgUnds.Underline sp MsgUnds.Primary [MsgUnds.Message MsgUnds.Error $ listname ++ ", if continued, needs " ++ delimiter ++ " after " ++ item ++ " (found " ++ Tokens.format_token found ++ " instead)"]
        ],
    [])
