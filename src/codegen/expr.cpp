...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::ShortCircuitExpr &ast) {
    ...
    if (!dynamic_cast<IR::BoolType const *>(&lhs.value->type())) {
        Errors::LHS_UNSUPPORTED_OP(lhs, ast.op.span);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }
    ...
    if (!dynamic_cast<IR::BoolType const *>(&rhs.value->type())) {
        Errors::CONFLICT_TYS_BINARY_OP(lhs, rhs, ast.op.span);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }
...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::DerefExpr &ast) {
    ...
    if (!asptrty) {
        Errors::NO_DEREF(ast.op.span, oper);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }

    ret = Located(ast, ir_builder->cur_block()->add<IR::Instrs::DerefPtr>(oper));
}
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::AddrofExpr &ast) {
    ...
    if (!as_register) {
        Errors::ADDROF_NOT_LVALUE(ast.op.span, oper);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }

    if (!as_register->mut && ast.mut) {
        Errors::MUT_ADDROF_NONMUT_OP(ast.op.span, *as_register);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }

    ret = Located(ast, ir_builder->cur_block()->add<IR::Instrs::Addrof>(*as_register, ast.mut));
}
...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::CallExpr &ast) {
    ...
    if (!fty) {
        Errors::CALL_NONCALLABLE(fun, ast.oparn.span);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }
    ...
    if (args.size() != fty->paramtys.size()) {
        Errors::WRONG_NUM_ARGS(*static_cast<IR::Function const *>(fun.value.as_raw()), *ast.callee, ast.oparn.span, args);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }
    ...
    ret = Located(ast, ir_builder->cur_block()->add<IR::Instrs::Call>(NNPtr<IR::Function const>(static_cast<IR::Function const *>(fun.value.as_raw())), args));
}

void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::BoolLit &ast) {
    ret = Located(ast, ir_builder->context().get_const_bool(ast.val.value.val));
}
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::FloatLit &ast) {
    ret = Located(ast, ir_builder->context().get_const_float(ir_builder->context().get_generic_float_type(), ast.val.value.val));
}
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::IntLit &ast) {
    ret = Located(ast, ir_builder->context().get_const_int(ir_builder->context().get_generic_int_type(), ast.val.value.val));
}
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::CharLit &ast) {
    ret = Located(ast, ir_builder->context().get_const_char(ast.val.value.val));
}
...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::ThisExpr &ast) {
    ...
    if (m_loc.has()) {
        ...
        ret = Located(ast, *local->v);
    } else {
        Errors::NO_THIS(ast.tok.span);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
    }
}
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::IfExpr &ast) {
    ...
    if (falseb.has()) {
        ...
        if (&truev.value->type() != &falsev.value->type()) {
            Errors::CONFL_TYS_IFEXPR(truev, falsev, ast.iftok.span, ast.elsetok.get().span);
            ret = Maybe<Located<NNPtr<IR::Value>>>();
            return;
        }
    } else {
        if (!dynamic_cast<IR::VoidType const *>(&truev.value->type())) {
            Errors::NO_ELSE_NOT_VOID(truev, ast.iftok.span);
            ret = Maybe<Located<NNPtr<IR::Value>>>();
            return;
        }
    }
...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::WhileExpr &ast) {
    ...
    ret = Located(ast, ir_builder->context().get_void());
}
...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::AssignmentExpr &ast) {
    ...
    if (!target_reg) {
        Errors::ASSIGN_INVALID_LHS(ast.equal.span, lhs);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }

    if (!target_reg->mut) {
        Errors::ASSIGN_NOT_MUT(rhs, ast.equal.span, *target_reg);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }
    ...
    if (&expect_type != &rhs.value->type()) {
        Errors::ASSIGN_CONFLICT_TYS(lhs, rhs, ast.equal.span);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }
...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::FieldAccessExpr &ast) {
    ...
    if (!has) {
        Errors::NO_FIELD(op, ast.field.span);
        return;
    }
...
void Codegen::Helpers::ExprCodegen::ast_visit(ASTNS::MethodCallExpr &ast) {
    ...
    if (!m_method.has()) {
        Errors::NO_METHOD(op, ast.method.span);
        return;
    }
    ...
    if (method.this_ptr) {
        ...
        if (!as_register) {
            Errors::ADDROF_NOT_LVALUE(ast.dot.span, op);
            ret = Maybe<Located<NNPtr<IR::Value>>>();
            return;
        }

        if (!as_register->mut && method.this_mut) {
            Errors::MUT_ADDROF_NONMUT_OP(ast.method.span, *as_register);
            ret = Maybe<Located<NNPtr<IR::Value>>>();
            return;
        }

        m_this_arg = Located(op.span, ir_builder->cur_block()->add<IR::Instrs::Addrof>(*as_register, method.this_mut));
    } else
        m_this_arg = op;
    ...
    if (args.size() != paramtys.size()) {
        Errors::WRONG_NUM_ARGS(*method.fun, ast, ast.oparn.span, args);
        ret = Maybe<Located<NNPtr<IR::Value>>>();
        return;
    }
    ...
    ret = Located(ast, ir_builder->cur_block()->add<IR::Instrs::Call>(method.fun, args));
}
