...
#define TRYINSERT(ty) if (tryInsert(ty, e.p, e.lookahead, e.stack)) fixes.push_back(fix {fix::fixtype::INSERT, ty});;
#define TRYSUB(ty) if (trySub(ty, e.p, e.lookahead, e.stack)) fixes.push_back(fix {fix::fixtype::SUBSTITUTE, ty});;
#define TRYTOKTY(ty) TRYINSERT(ty); TRYSUB(ty);
    ...
    if (tryDel(e.p, e.stack)) fixes.push_back(fix {fix::fixtype::REMOVE});
// This code was autogenerated - see the utils/ directory
    ...
    if (fixes.size())
    {
        applyFix(fixes.front(), e.p, e.lookahead);
