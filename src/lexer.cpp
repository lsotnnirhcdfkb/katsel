...
// begin autogenerated code from utils/kwgen.py
                ...
                case 'n':
                    switch (*(start + 2)) {
                        case 'l':
                            ...
                            break;
                        case 't':
                            if (start + 3 == end) return TokenType::INT;
                            break;
                    }
                    break;
            }
...
// end autogenerated code from utils/kwgen.py
...
Token Lexer::nextToken() {
    ...
    if (isDigit(c)) { // check for number literal
        ...
        if (peek() == '.' && isDigit(peekpeek(), inttype) && !atEnd()) { // is actually a decimal and is not integer literal
            ...
            if (inttype != TokenType::DECINTLIT) return makeErrorToken("Error lexing: Non-decimal floating point literals are not supported");
