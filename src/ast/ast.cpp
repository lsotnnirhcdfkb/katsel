...
ASTNS::FunctionDecl::FunctionDecl(Span const &span, std::unique_ptr<Type> retty, Located<Tokens::Identifier> name, std::vector<std::unique_ptr<ParamB>> params, std::unique_ptr<Block> body): _span(span), retty(std::move(retty)), name(std::move(name)), params(std::move(params)), body(std::move(body)) {}
...
ASTNS::VarStmt::VarStmt(Span const &span, std::unique_ptr<Type> type, bool mut, Located<Tokens::Identifier> name, Maybe<Located<Tokens::Equal>> equal, std::unique_ptr<Expr> expr): _span(span), type(std::move(type)), mut(std::move(mut)), name(std::move(name)), equal(std::move(equal)), expr(std::move(expr)) {}
...
ASTNS::ThisType::ThisType(Span const &span, Located<Tokens::This> th): _span(span), th(std::move(th)) {}
void ASTNS::ThisType::ast_accept(ASTNS::TypeVisitor &v) { v.ast_visit(*this); }
Span const &ASTNS::ThisType::span() const { return _span; }
ASTNS::Param::Param(Span const &span, std::unique_ptr<Type> type, Located<Tokens::Identifier> name, bool mut): _span(span), type(std::move(type)), name(std::move(name)), mut(std::move(mut)) {}
...
ASTNS::IfExpr::IfExpr(Span const &span, Located<Tokens::If> iftok, Maybe<Located<Tokens::Else>> elsetok, std::unique_ptr<Expr> cond, std::unique_ptr<Expr> trues, std::unique_ptr<Expr> falses): _span(span), iftok(std::move(iftok)), elsetok(std::move(elsetok)), cond(std::move(cond)), trues(std::move(trues)), falses(std::move(falses)) {}
...
ASTNS::AddrofExpr::AddrofExpr(Span const &span, Located<Tokens::Amper> op, std::unique_ptr<Expr> expr, bool mut): _span(span), op(std::move(op)), expr(std::move(expr)), mut(std::move(mut)) {}
void ASTNS::AddrofExpr::ast_accept(ASTNS::ExprVisitor &v) { v.ast_visit(*this); }
Span const &ASTNS::AddrofExpr::span() const { return _span; }
ASTNS::DerefExpr::DerefExpr(Span const &span, Located<Tokens::Star> op, std::unique_ptr<Expr> expr): _span(span), op(std::move(op)), expr(std::move(expr)) {}
void ASTNS::DerefExpr::ast_accept(ASTNS::ExprVisitor &v) { v.ast_visit(*this); }
Span const &ASTNS::DerefExpr::span() const { return _span; }
ASTNS::CallExpr::CallExpr(Span const &span, std::unique_ptr<Expr> callee, Located<Tokens::OParen> oparn, std::vector<std::unique_ptr<Expr>> args): _span(span), callee(std::move(callee)), oparn(std::move(oparn)), args(std::move(args)) {}
void ASTNS::CallExpr::ast_accept(ASTNS::ExprVisitor &v) { v.ast_visit(*this); }
Span const &ASTNS::CallExpr::span() const { return _span; }
ASTNS::FieldAccessExpr::FieldAccessExpr(Span const &span, std::unique_ptr<Expr> operand, Located<Tokens::Period> dot, Located<Tokens::Identifier> field): _span(span), operand(std::move(operand)), dot(std::move(dot)), field(std::move(field)) {}
...
ASTNS::Path::Path(Span const &span, std::vector<Located<Tokens::Identifier>> segments): _span(span), segments(std::move(segments)) {}
