...
#include "ast/ast.h"
ASTNS::CU::CU(std::unique_ptr<Decl> decls):decls(std::move(decls)) {}
void ASTNS::CU::accept(ASTNS::CUB::Visitor *v) { v->visitCU(this); }
ASTNS::FunctionDecl::FunctionDecl(std::unique_ptr<Type> retty, Token name, std::vector<std::unique_ptr<Param>> params):retty(std::move(retty)), name(std::move(name)), params(std::move(params)) {}
void ASTNS::FunctionDecl::accept(ASTNS::Decl::Visitor *v) { v->visitFunctionDecl(this); }
ASTNS::VarStmt::VarStmt(std::unique_ptr<Type> type, std::vector<std::unique_ptr<VarStmtItem>> assignments):type(std::move(type)), assignments(std::move(assignments)) {}
...
ASTNS::Block::Block(std::vector<std::unique_ptr<Stmt>> stmts, std::unique_ptr<Expr> implRet):stmts(std::move(stmts)), implRet(std::move(implRet)) {}
...
ASTNS::CallExpr::CallExpr(std::unique_ptr<Expr> callee, Token oparn, std::vector<std::unique_ptr<Arg>> args):callee(std::move(callee)), oparn(std::move(oparn)), args(std::move(args)) {}
