...
# instructions {{{1
instructions = [
    Instruction('Store'       , 'target.type()->context.getVoidType()'           , ['ASTValue target', 'ASTValue value', 'bool init'], [type_must_be('target.type()', 'PointerType'), 'static_cast<PointerType*>(target.type())->ty == value.type()', 'init || static_cast<PointerType*>(target.type())->mut']),
    Instruction('Phi'         , 'prevs[0].second.type()'                         , ['std::vector<std::pair<Block*,ASTValue>> prevs' ], ['prevs.size() > 0']),

    Instruction('Register'    , 'ty->context.getPointerType(mut, ty)'            , ['Type* ty', 'bool mut'                          ], [], declared=True),
    ...
    Instruction('Addrof'      , 'deref->type()->context.getPointerType(mut, deref->type())', ['DerefPtr* deref', 'bool mut'         ], ['static_cast<int>(mut) <= static_cast<int>(static_cast<PointerType*>(deref->ptr.type())->mut)']),
