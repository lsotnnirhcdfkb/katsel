...
def type_must_be(var_type, must_be):
    return f'dynamic_cast<{must_be}*>({var_type}.as_raw())'
...
# instructions {{{1
instructions = [
    Instruction('Store'       , 'target.type()->context.get_void_type()'           , ['ASTValue target', 'ASTValue value', 'bool init'      ], [type_must_be('target.type()', 'PointerType'), 'static_cast<PointerType*>(target.type().as_raw())->ty == value.type()', 'init || static_cast<PointerType*>(target.type().as_raw())->mut']),
    Instruction('Phi'         , 'prevs[0].second.type()'                         , ['std::vector<std::pair<NNPtr<Block>,ASTValue>> prevs' ], ['prevs.size() > 0']),

    Instruction('Register'    , 'ty->context.get_pointer_type(mut, ty)'            , ['NNPtr<Type> ty', 'bool mut'                          ], [], declared=True),

    Instruction('Or'          , 'lhs.type()->context.get_bool_type()'              , ['ASTValue lhs', 'ASTValue rhs'                        ], [type_must_be('lhs.type()', 'BoolType'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('And'         , 'lhs.type()->context.get_bool_type()'              , ['ASTValue lhs', 'ASTValue rhs'                        ], [type_must_be('lhs.type()', 'BoolType'), *operands_equal('lhs.type()', 'rhs.type()')]),
    ...
    Instruction('Addrof'      , 'deref->type()->context.get_pointer_type(mut, deref->type())', ['NNPtr<DerefPtr> deref', 'bool mut'         ], ['static_cast<int>(mut) <= static_cast<int>(static_cast<PointerType*>(deref->ptr.type().as_raw())->mut)']),
    Instruction('DerefPtr'    , 'static_cast<PointerType*>(ptr.type().as_raw())->ty', ['ASTValue ptr'                                      ], [type_must_be('ptr.type()', 'PointerType')]),
    ...
    Br('CondBr'      , ['ASTValue v', 'NNPtr<Block> true_b', 'NNPtr<Block> false_b'    ], [type_must_be('v.type()', 'BoolType')]),
]
...
def gen_decls():
    ...
    for instruction in instructions:
        ...
        if instruction.declared:
            output.append(  '            NNPtr<ASTNS::AST> def_ast() const override;\n')
...
def gen_defs():
    ...
    for instruction in instructions:
        ...
        if instruction.base() == 'Instruction':
            ...
            if instruction.declared:
                output.append(f'NNPtr<ASTNS::AST> IR::Instrs::{instruction.name}::def_ast() const {{ return _def_ast; }}\n')
