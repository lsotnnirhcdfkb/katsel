...
# instructions {{{1
instructions = [
    Instruction('Store'       , 'target.type().context.get_void_type()'           , ['ASTValue|target', 'ASTValue|value', 'bool|init'      ], [type_must_be('target.type()', 'PointerType const'), 'static_cast<PointerType const *>(&target.type())->ty.as_raw() == &value.type()', 'init || static_cast<PointerType const *>(&target.type())->mut']),
    Instruction('Phi'         , 'prevs[0].second.type()'                         , ['std::vector<std::pair<NNPtr<Block const>, ASTValue>>|prevs' ], ['prevs.size() > 0']),

    Instruction('Register'    , 'ty->context.get_pointer_type(mut, *ty)'            , ['NNPtr<Type const>|ty', 'bool|mut'                          ], [], declared=True),

    Instruction('Or'          , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_must_be('lhs.type()', 'BoolType const'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('And'         , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_must_be('lhs.type()', 'BoolType const'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('Not'         , 'op.type().context.get_bool_type()'               , ['ASTValue|op'                                         ], []),

    Instruction('ICmpNE'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [*operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('ICmpEQ'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [*operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('ICmpLT'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [*operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('ICmpGT'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [*operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('ICmpLE'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [*operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('ICmpGE'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [*operands_equal('lhs.type()', 'rhs.type()')]),
    ...
    Instruction('FCmpNE'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_is_floating('lhs.type()'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('FCmpEQ'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_is_floating('lhs.type()'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('FCmpLT'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_is_floating('lhs.type()'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('FCmpGT'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_is_floating('lhs.type()'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('FCmpLE'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_is_floating('lhs.type()'), *operands_equal('lhs.type()', 'rhs.type()')]),
    Instruction('FCmpGE'      , 'lhs.type().context.get_bool_type()'              , ['ASTValue|lhs', 'ASTValue|rhs'                        ], [type_is_floating('lhs.type()'), *operands_equal('lhs.type()', 'rhs.type()')]),
    ...
    Instruction('NoOpCast'    , '*newt'                                           , ['ASTValue|op', 'NNPtr<Type const>|newt'                     ], []),

    Instruction('IntToInt'    , '*newt'                                           , ['ASTValue|op', 'NNPtr<IntType const>|newt'                  ], [type_is_integral('op.type()')]),
    Instruction('IntToFloat'  , '*newt'                                           , ['ASTValue|op', 'NNPtr<FloatType const>|newt'                ], [type_is_integral('op.type()')]),
    Instruction('FloatToFloat', '*newt'                                           , ['ASTValue|op', 'NNPtr<FloatType const>|newt'                ], [type_is_floating('op.type()')]),
    Instruction('FloatToInt'  , '*newt'                                           , ['ASTValue|op', 'NNPtr<IntType const>|newt'                  ], [type_is_floating('op.type()')]),

    Instruction('Call'        , '*f->ty->ret'                                     , ['NNPtr<Function const>|f', 'std::vector<ASTValue>|args'     ], ['args.size() == f->ty->paramtys.size()']),

    Instruction('Addrof'      , 'deref->type().context.get_pointer_type(mut, deref->type())', ['NNPtr<DerefPtr const>|deref', 'bool|mut'         ], ['static_cast<int>(mut) <= static_cast<int>(static_cast<PointerType const *>(&deref->ptr.type())->mut)']),
    Instruction('DerefPtr'    , '*static_cast<PointerType const *>(&ptr.type())->ty', ['ASTValue|ptr'                                      ], [type_must_be('ptr.type()', 'PointerType const')]),
    Instruction('PtrArith'    , 'ptr.type()'                                     , ['ASTValue|ptr', 'ASTValue|offset'                     ], [type_must_be('ptr.type()', 'PointerType const'), type_is_integral('offset.type()')]),

    Br('Return'      , ['ASTValue|value'                                             ], []),
    Br('GotoBr'      , ['NNPtr<Block>|to'                                            ], []),
    Br('CondBr'      , ['ASTValue|v', 'NNPtr<Block>|true_b', 'NNPtr<Block>|false_b'    ], [type_must_be('v.type()', 'BoolType const')]),
]
