...
namespace msg
{
    ...
    inline std::string invalidROperand(Value lop, Token op, Type *rty)
    {
        std::stringstream ss;
        ss << "Invalid right operand of type \"" << rty->stringify() << "\" to operator \"" << tokenToStr(op) << "\" with left operand of type \"" << lop.type->stringify() << "\"";
        return ss.str();
    }
